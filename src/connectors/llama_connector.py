"""
Connecteur pour le modèle Llama3, facilitant l'interaction avec Ollama
"""
...
logger = logging.getLogger("LlamaConnector")

class LlamaConnector:
    """
    Classe pour interagir avec le modèle Llama3 via Ollama
    """
    
    def __init__(self, base_url: str = "http://localhost:11434/api", model_name: str = "llama3:latest"):
        """
        Initialise le connecteur Llama3.
        
        Args:
            base_url: URL de base pour l'API Ollama
            model_name: Nom du modèle Llama à utiliser
        """
        self.base_url = base_url
        self.model_name = model_name
        self.generate_endpoint = f"{self.base_url}/generate"
        logger.info(f"LlamaConnector initialisé avec le modèle {model_name}")
    ...
    def generate(self, 
        ...
        """
        Génère une réponse à partir du modèle Llama3.
        ...
        try:
            logger.info(f"Envoi de requête à Llama3 (taille prompt: {len(prompt)} caractères)")
            response = requests.post(self.generate_endpoint, headers=headers, data=json.dumps(payload))
            
            if response.status_code == 200:
                result = response.json()
                generated_text = result.get("response", "")
                logger.info(f"Réponse reçue de Llama3 (taille: {len(generated_text)} caractères)")
...
if __name__ == "__main__":
    ...
    if llama.check_availability():
        response = llama.get_completion("Qui est Chaka dans la tradition africaine?")
        print(f"Réponse: {response}")
    else:
        print("Modèle Llama3 non disponible. Vérifiez qu'Ollama est en cours d'exécution et que le modèle est installé.")