...
if __name__ == "__main__":
    # Obtenir le chemin du projet (dossier parent du dossier courant)
    current_dir = os.path.dirname(os.path.abspath(__file__))
    project_path = os.path.dirname(os.path.dirname(os.path.dirname(current_dir)))
    
    print(f"Initialisation de Mbongi avec le chemin de projet: {project_path}")
    
    # Créer une instance de Mbongi
    mbongi = Mbongi(project_path)
    
    # Tester le démarrage d'une session
    mbongi.start_session()
    
    # Tester le traitement d'une idée
    idea = """
[MBONGI:IDEA]
Title: Amélioration de l'analyse de code
Component: Mbongi
Type: Amélioration
Priority: Moyenne
Description:
Ajouter une analyse de la qualité du code basée sur des règles prédéfinies.
Cela permettrait d'identifier automatiquement les zones de code qui nécessitent
une refactorisation.

Implementation:
Utiliser des bibliothèques comme Pylint ou Flake8 pour analyser le code.
Intégrer les résultats dans les fiches techniques des composants.
...
[/MBONGI:IDEA]
"""
    mbongi.process_idea(idea)
    
    # Tester l'analyse d'un composant
    try:
        component_path = os.path.join("src", "agents", "documentation", "mbongi.py")
        if os.path.exists(os.path.join(project_path, component_path)):
            print(f"Analyse du composant: {component_path}")
            mbongi.analyze_component(component_path)
        else:
            print(f"Le composant {component_path} n'existe pas encore, analyse ignorée.")
    except Exception as e:
        print(f"Erreur lors de l'analyse: {str(e)}")
    
    # Terminer la session
    summary = mbongi.end_session()
    if summary:
        print("Résumé de la session:", json.dumps(summary, indent=2))
    
    print("Test de Mbongi terminé.")