# ----- START OF FILE src/tools/signal_utils.py (Version Complète - Permet Englobement Dojis) -----
import pandas as pd
import numpy as np
...
# ... (Code inchangé pour calculate_zigzag_pivots) ...
...
# ... (Code inchangé pour find_interest_zone) ...
...
# ... (Code inchangé pour check_divergence_in_zone - avec tolérances) ...
def check_divergence_in_zone(current_low: float, current_high: float, current_rsi: float, zone_info: dict, last_relevant_pivot: dict, rsi_at_last_pivot: float) -> str | None:
    if zone_info is None or last_relevant_pivot is None or pd.isna(current_rsi) or pd.isna(rsi_at_last_pivot): return None
    zone_start = zone_info['start_price']; zone_end = zone_info['end_price']; direction = zone_info['direction']; pivot_price = last_relevant_pivot['price']
    zone_upper = max(zone_start, zone_end); zone_lower = min(zone_start, zone_end)
    price_tolerance_relative = pivot_price * 0.0005; price_tolerance_absolute = 0.1
    price_tolerance = max(price_tolerance_relative, price_tolerance_absolute); rsi_tolerance = 1.0
    if direction == 'bullish':
        if last_relevant_pivot['type'] != 'low': return None
        if not (zone_lower <= current_low <= zone_upper): return None
        if current_low < pivot_price - price_tolerance and current_rsi > rsi_at_last_pivot + rsi_tolerance: return "BULL_REGULAR"
        if current_low > pivot_price + price_tolerance and current_rsi < rsi_at_last_pivot - rsi_tolerance: return "BULL_CONTINUATION"
    elif direction == 'bearish':
        if last_relevant_pivot['type'] != 'high': return None
        if not (zone_lower <= current_high <= zone_upper): return None
        if current_high > pivot_price + price_tolerance and current_rsi < rsi_at_last_pivot - rsi_tolerance: return "BEAR_REGULAR"
        if current_high < pivot_price - price_tolerance and current_rsi > rsi_at_last_pivot + rsi_tolerance: return "BEAR_CONTINUATION"
    return None
...
def detect_engulfing_pattern(df_ohlc: pd.DataFrame, current_candle_index: pd.Timestamp, direction: str) -> bool:
    ...
    MODIFIÉ: Permet l'englobement des dojis/petits corps.
    ...
    try:
        ...
        # body_tolerance = (df_ohlc['High'].mean() - df_ohlc['Low'].mean()) * 0.01 if len(df_ohlc)>0 else 0.01
        # ===> LIGNE COMMENTÉE CI-DESSOUS POUR PERMETTRE ENGLOBEMENT DOJI <===
...
# ... (Code inchangé pour calculate_stop_loss) ...
def calculate_stop_loss(df_ohlc: pd.DataFrame, confirmation_candle_index: pd.Timestamp, direction: str, buffer_points: float = 5.0, max_lookback_candles: int = 10) -> float | None:
    try:
        ...
        start_range_loc = -1; search_start_loc = confirmation_loc - 1
        ...
        for i in range(search_start_loc, lookback_limit -1, -1):
            ...
            is_bullish = candle['Close'] > candle['Open']; is_bearish = candle['Close'] < candle['Open']
        ...
        if sl_range_df.empty or sl_range_df[['Low', 'High']].isnull().values.any(): return None
        stop_loss_level = None
        if direction == 'bullish': stop_loss_level = sl_range_df['Low'].min() - buffer_points
        elif direction == 'bearish': stop_loss_level = sl_range_df['High'].max() + buffer_points
...
# ... (Code inchangé pour calculate_take_profit_v1) ...
def calculate_take_profit_v1(pivots: list, confirmation_candle_index: pd.Timestamp, direction: str, divergence_type: str | None, zone_info: dict | None, df_ohlc: pd.DataFrame = None) -> float | None:
    ...
    if "CONTINUATION" in divergence_type:
        ...
        for pivot in reversed(pivots):
            ...
            if pivot_index is None or pivot.get('type') is None or pivot.get('price') is None or pivot_index >= confirmation_candle_index: continue
            if pivot['type'] == target_pivot_type: target_pivot = pivot; break
...
def calculate_position_size(account_balance: float, risk_percentage: float, sl_level: float, entry_price: float, symbol_info: dict) -> float | None:
    try:
        ...
        try:
            point = symbol_info['point']; tick_value = symbol_info['trade_tick_value']; tick_size = symbol_info['trade_tick_size']
            volume_min = symbol_info['volume_min']; volume_max = symbol_info['volume_max']; volume_step = symbol_info['volume_step']
        except KeyError as ke: print(f"DEBUG Size: Information symbole manquante: {ke}."); return None
        if None in [point, tick_value, tick_size, volume_min, volume_max, volume_step] or \
           tick_size <= 0 or point <= 0 or volume_step <= 0: print(f"DEBUG Size: Informations symbole numériques invalides."); return None
        risk_amount = account_balance * (risk_percentage / 100.0); sl_distance_price = abs(entry_price - sl_level)
        ...
        volume_ideal = risk_amount / (sl_distance_points * point_value_per_lot); epsilon = 1e-9
        volume_adjusted = math.floor(volume_ideal / volume_step + epsilon) * volume_step
        volume_final = max(volume_min, volume_adjusted); volume_final = min(volume_max, volume_final)
        min_volume_risk = (sl_distance_points * point_value_per_lot) * volume_min
        if min_volume_risk > risk_amount + epsilon: print(f"ATTENTION Size: Vol min ({volume_min}) risque ({min_volume_risk:.2f}) > Risque autorisé ({risk_amount:.2f})."); pass
        decimals = 0
        if '.' in str(volume_step):
             try: decimals = abs(math.log10(volume_step)); decimals = int(decimals) if decimals == math.floor(decimals) else len(str(volume_step).split('.')[-1])
             except ValueError: print(f"DEBUG Size: Impossible de déterminer les décimales pour volume_step={volume_step}"); return None
        volume_final = round(volume_final, decimals)
        if volume_final < volume_min: volume_final = volume_min
